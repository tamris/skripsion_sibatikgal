...
class DetailHistoryView extends StatelessWidget {
  ...
  double _parseConfidence() {
    final rawConfidence = historyData['akurasi'] ?? historyData['confidence'] ?? '0%';
  ...
  Future<void> _openHistoryMotifDetail() async {
    ...
    if (targetMotifName.isEmpty) {
      Get.snackbar('Motif tidak tersedia', 'Data riwayat tidak memiliki nama motif yang valid.', snackPosition: SnackPosition.BOTTOM);
      return;
    }
    ...
    if (batik == null) {
      Get.snackbar('Detail tidak ditemukan', 'Motif "$targetMotifName" belum tersedia di data galeri.', snackPosition: SnackPosition.BOTTOM);
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final String akurasiLabel = historyData['akurasi'] ?? historyData['confidence'] ?? '${(_parseConfidence() * 100).toStringAsFixed(0)}%';
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              ...
              child: Container(color: DeteksiColors.cKrem, child: _buildHeader(context)),
            ),
            Positioned(
          ...
          child: fullImageUrl.isNotEmpty
              ? Image.network(
                  ...
                  loadingBuilder: (context, child, loadingProgress) {
                    ...
                    return const Center(child: CircularProgressIndicator(color: DeteksiColors.cBrown));
                  },
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Text(akurasiLabel, style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w500, color: pctColor)),
                  ],
                ),
            ...
            Expanded(
              child: Text.rich(TextSpan(
                ...
                children: [
                  ...
                  TextSpan(text: tanggalRelatif, style: GoogleFonts.poppins(fontWeight: FontWeight.w600)),
                ],
              ))),
          ],
        ),
  ...
  Widget _buildBottomCta(BuildContext context, double bottomPadding) {
    return Container(
      ...
      child: _isLowConfidence ? _buildCtaLowConfidence(context) : _buildCtaHighConfidence(context),
    );
  }
  ...
  Widget _buildCtaHighConfidence(BuildContext context) {
    return Row(
      children: [
        SmallBtn(icon: Icons.refresh_rounded, onTap: () => Navigator.pop(context)),
        const SizedBox(width: 8),
        Expanded(child: PrimaryBtn(icon: Icons.menu_book_rounded, label: 'Lihat Detail Motif', onTap: _openHistoryMotifDetail)),
      ],
    );
  }
  ...
  Widget _buildCtaLowConfidence(BuildContext context) {
    return Row(
      children: [
        Expanded(child: PrimaryBtn(icon: Icons.refresh_rounded, label: 'Deteksi Ulang', onTap: () => Navigator.pop(context))),
        const SizedBox(width: 8),
